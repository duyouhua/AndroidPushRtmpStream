...
#include "com_ytx_push_FFmpegUtils.h"
        ...
        (JNIEnv *env, jclass clazz, jstring input_jstr, jstring output_jstr)
{
    ...
    //java string->c char*
    const char *input_cstr = env->GetStringUTFChars(input_jstr, JNI_FALSE);
    const char *output_cstr = env->GetStringUTFChars(output_jstr, JNI_FALSE);
    //变量初始化
